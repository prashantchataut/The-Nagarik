...
| # | Finding | Fix |
|---|---------|-----|
| ✅🔴1 | ~~Reader account does not power features~~ **CLOSED 2026-08-12**: server-synced library (`readers.savedStories/readingHistory` + tombstones, `/api/reader/library` GET/PUT/DELETE, merge = union-by-storyId newest-wins; E2E incl. anti-resurrection), client sync layer (`components/account/library-sync.ts`) wired into bookmark toggle, reading progress, both panels, login/register/reset; homepage `ForYouStrip` reads account interests (client-side, ISR-safe). ADR 0007. Still open: comment identity pre-fill from session. |
| ✅🔴2 | ~~No password reset / email adapter~~ **CLOSED 2026-08-12**: `@payloadcms/email-nodemailer` behind SMTP_* env (console fallback in dev, `emailConfigured` in /api/health), bilingual branded reset emails, `/api/reader/forgot-password` (anti-enumeration, rate-limited) + `/api/reader/reset-password` (auto-login) + `/{locale}/forgot-password|reset-password` pages + login-form link; journalist approval sends set-your-password invite when SMTP is up (`inviteEmailSent`), manual handover fallback kept. Full loop verified by `scripts/verify-password-reset.ts`. ADR 0007. Still open: email change flow, staff-initiated reset UI. |
